    ...
    $('a.pane-delete:not(.pane-delete-processed)', context)
      ...
      .click(function() {
        if (confirm(Drupal.t('Remove this pane?'))) {
          $(this).parents('div.dpidestinations-ipe-portlet-wrapper').fadeOut('medium', function() {
            var $sortable = $(this).closest('.ui-sortable');
            $(this).empty().remove();
            $sortable.trigger('sortremove');
          });
          $(this).parents('div.dpidestinations-ipe-display-container').addClass('changed');
        }
        return false;
      });
  }
...
Drupal.behaviors.PanelsIPE = {
  attach: function(context) {
    for (var i in Drupal.settings.PanelsIPECacheKeys) {
      ...
      $('div#dpidestinations-ipe-display-' + key + ':not(.dpidestinations-ipe-processed)')
        .addClass('dpidestinations-ipe-processed')
        .each(function() {
    ...
    $('.dpidestinations-ipe-hide-bar').once('dpidestinations-ipe-hide-bar-processed').click(function() {
...
function DrupalPanelsIPE(cache_key, cfg) {
  ...
  this.lockPath = null;
  this.state = {};
  this.container = $('#dpidestinations-ipe-control-container');
  this.control = $('div#dpidestinations-ipe-control-' + cache_key);
  this.initButton = $('div.dpidestinations-ipe-startedit', this.control);
  this.cfg = cfg;
  this.changed = false;
  this.sortableOptions = $.extend({
    opacity: 0.75, // opacity of sortable while sorting
    items: 'div.dpidestinations-ipe-portlet-wrapper',
    handle: 'div.dpidestinations-ipe-draghandle',
    cancel: '.dpidestinations-ipe-nodrag',
    dropOnEmpty: true
  }, cfg.sortableOptions || {});
  ...
  $(document).bind('CToolsDetachBehaviors', function() {
    ...
    if (!$('.dpidestinations-ipe-form-container', ipe.control).html() && !ipe.container.is(':visible')) {
      ipe.showContainer();
      ipe.cancelLock();
    }
    ...
    // re-display it.
    if (ipe.topParent && ipe.topParent.hasClass('dpidestinations-ipe-editing') && ipe.container.is(':not(visible)')) {
      ipe.showContainer();
    }
  });
  ...
  this.cancelLock = function(sync) {
    ...
    if (ipe.lockPath && Drupal.ajax['dpidestinations-ipe-customize-page']) {
      var ajaxOptions = {
        type: 'POST',
        url: ipe.lockPath
      }
  ...
  this.activateSortable = function(event, ui) {
    ...
    var paneId = ui.item.attr('id').replace('dpidestinations-ipe-paneid-', '');
  ...
  this.initSorting = function() {
    var $region = $(this).parents('.dpidestinations-ipe-region');
    var region = $region.attr('id').replace('dpidestinations-ipe-regionid-', '');
  ...
  this.initEditing = function(formdata) {
    ipe.editing = true;
    ipe.topParent = $('div#dpidestinations-ipe-display-' + cache_key);
    ...
    $('div.dpidestinations-ipe-sort-container', ipe.topParent).each(ipe.initSorting);

    // Since the connectWith option only does a one-way hookup, iterate over
    // all sortable regions to connect them with one another.
    $('div.dpidestinations-ipe-sort-container', ipe.topParent)
      .sortable('option', 'connectWith', ['div.dpidestinations-ipe-sort-container']);

    $('div.dpidestinations-ipe-sort-container', ipe.topParent).bind('sortupdate', function() {
      ipe.changed = true;
    });

    $('div.dpidestinations-ipe-sort-container', ipe.topParent).bind('sortstop', this.enableRegions);

    $('.dpidestinations-ipe-form-container', ipe.control).append(formdata);
    ...
    $('.dpidestinations-ipe-on').show();
    ipe.showForm();
    ipe.topParent.addClass('dpidestinations-ipe-editing');
  ...
  this.showButtons = function() {
    $('.dpidestinations-ipe-form-container').hide();
    $('.dpidestinations-ipe-button-container').show();
    ipe.showContainer();
  };

  this.showForm = function() {
    $('.dpidestinations-ipe-button-container').hide();
    $('.dpidestinations-ipe-form-container').show();
    ipe.showContainer();
  };
  ...
  this.endEditing = function() {
    ipe.editing = false;
    ipe.lockPath = null;
    $('.dpidestinations-ipe-form-container').empty();
    // Re-show all the IPE non-editing meta-elements
    $('div.dpidestinations-ipe-off').show('fast');

    ipe.showButtons();
    // Re-hide all the IPE meta-elements
    $('div.dpidestinations-ipe-on').hide();

    $('.dpidestinations-ipe-editing').removeClass('dpidestinations-ipe-editing');
    $('div.dpidestinations-ipe-sort-container.ui-sortable', ipe.topParent).sortable("destroy");
  };

  this.saveEditing = function() {
    $('div.dpidestinations-ipe-region', ipe.topParent).each(function() {
      var val = '';
      var region = $(this).attr('id').split('dpidestinations-ipe-regionid-')[1];
      $(this).find('div.dpidestinations-ipe-portlet-wrapper').each(function() {
        var id = $(this).attr('id').split('dpidestinations-ipe-paneid-')[1];
        if (id) {
          if (val) {
            val += ',';
          }
  ...
  this.cancelIPE = function() {
    ...
    ipe.topParent.fadeOut('medium', function() {
      ...
      ipe.topParent = $('div#dpidestinations-ipe-display-' + ipe.key);
  ...
  this.createSortContainers = function() {
    $('div.dpidestinations-ipe-region', this.topParent).each(function() {
      $(this).children('div.dpidestinations-ipe-portlet-marker').parent()
        .wrapInner('<div class="dpidestinations-ipe-sort-container" />');

      // Move our gadgets outside of the sort container so that sortables
      // cannot be placed after them.
      $('div.dpidestinations-ipe-portlet-static', this).each(function() {
...
$(function() {
  ...
  Drupal.ajax.prototype.commands.insertNewPane = function(ajax, data, status) {
    IPEContainerSelector = '#dpidestinations-ipe-regionid-' + data.regionId + ' div.dpidestinations-ipe-sort-container';
    firstPaneSelector = IPEContainerSelector + ' div.dpidestinations-ipe-portlet-wrapper:first';
  ...
  Drupal.ajax.prototype.eventResponse = function (element, event) {
    ...
    if ($(this.element).attr('id') == 'dpidestinations-ipe-cancel') {
    ...
    // @TODO $('#dpidestinations-ipe-throbber-backdrop').remove();
    return retval;
  };
  ...
  Drupal.ajax.prototype.beforeSerialize = function (element_settings, options) {
    if ($(this.element).hasClass('dpidestinations-ipe-save')) {
...
Drupal.behaviors.panelsIpeMarginBottom = {
  attach: function () {
    $('body:not(.dpidestinations-ipe)').addClass('dpidestinations-ipe');
  }
};
